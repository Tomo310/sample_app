...
(function() {
  $(function() {
    return $('#micropost_content').on('keyup keydown keypress change', function() {
      var limit, thisValueLength;
      thisValueLength = $(this).val().length;
      limit = 140;
      if (thisValueLength < limit) {
        return $('.count').html("残り" + (limit - thisValueLength) + "文字").removeClass("text-danger");
      } else {
        return $('.count').html("残り" + (limit - thisValueLength) + "文字").addClass("text-danger");
      }
    }).keyup();
  });
